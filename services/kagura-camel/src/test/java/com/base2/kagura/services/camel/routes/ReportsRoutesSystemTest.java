...
public class ReportsRoutesSystemTest extends CamelSpringTestSupport {
    ...
    public void reportDetailsAndRunTest() {
        ...
        expect().body("extra.displayPriority", equalTo("1"))
                ...
                .body("rows", hasSize(2))
                .when()
                .get("http://localhost:8432/report/{1}/fake1/detailsAndRun", token);
    }
    ...
    public void reportDetailsNoRightsTest() {
        ResponseBody login = given().request()
                ...
                .body();
        String token = login.jsonPath().get("token");
        expect().body("error", equalTo("No such report"))
                .body("reportId", equalTo("fake2"))
                .when()
                .get("http://localhost:8432/report/{1}/fake2/details", token);
    }
    ...
    public void reportDetailsNoReportTest() {
        ResponseBody login = given().request()
                ...
                .body();
        String token = login.jsonPath().get("token");
        expect().body("reportId", equalTo("fake-1"))
                .body("error", equalTo("No such report"))
                .when()
                .get("http://localhost:8432/report/{1}/fake-1/details", token);
    }
    ...
    public void reportRunTest() {
        ResponseBody login = given().request()
                ...
                .body();
        String token = login.jsonPath().get("token");
        expect().body("columns", hasSize(3))
                .body("rows", hasSize(2))
                .when()
                .get("http://localhost:8432/report/{1}/fake1/run", token);
    }
    ...
    public void reportExportCSVParamsTest() throws JsonProcessingException, EncoderException {
        ...
        ResponseBody responseBody =
                expect().when().get("http://localhost:8432/report/{1}/fake1/export.csv?parameters={2}", token, param);
        byte[] bytes = responseBody.asByteArray();
        Assert.assertThat(bytes.length, equalTo(54));
    }
    ...
    public void reportExportPDFParamsTest() throws JsonProcessingException, EncoderException {
        ...
        String param = buildParameters();
        ResponseBody responseBody =
                expect().when().get("http://localhost:8432/report/{1}/fake1/export.pdf?parameters={2}", token, param);
        byte[] bytes = responseBody.asByteArray();
        Assert.assertThat(bytes.length, lessThanOrEqualTo(1220));
        Assert.assertThat(bytes.length, greaterThanOrEqualTo(1200));
    }
    ...
    public void reportExportXLSParamsTest() throws JsonProcessingException, EncoderException {
        ...
        String param = buildParameters();
        ResponseBody responseBody =
                expect().when().get("http://localhost:8432/report/{1}/fake1/export.xls?parameters={2}", token, param);
        byte[] bytes = responseBody.asByteArray();
        Assert.assertThat(bytes.length, equalTo(4096));
    }
